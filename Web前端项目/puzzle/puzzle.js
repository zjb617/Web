...
d[8] = 8;
d[9] = 0;
//默认按照顺序拍排好,大DIV第九块没有,所以为0,我们用0表示空白快
...
function move(id) {
    ...
    //设置游戏是否完成的标志,true表示完成
    for (var k = 1; k < 9; ++k) {
        if (d[k] != k) {
            finish_flag = false;
            break;
            //如果大DIV保存的编号和它本身的编号不同,则表示还不是全部按照顺序排的,那么设置为false,跳出循环,后面就不用再判断了,因为只要一个不符,就没完成游戏
        }
    }
...
function whereCanTo(cur_div) {
    ...
    for (j = 0; j < d_direct[cur_div].length; ++j) {
        ...
        //如果目标值为0,说明目标设置没有装小DIV,则可以移动,跳出循环
    }
    if (move_flag == true) {
...
function start() {
    if (pause) {
        ...
        set_timer = setInterval(timer, 1000); //自动定时
        //如果当前是暂停,则开始
    } else {
